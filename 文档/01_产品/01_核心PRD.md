...
| 当前交付目标 | 从任务要求和原始资料生成可追溯的建筑数据、三维模型、测绘图纸与成果包；完整流程用于求职作品展示和商业演示，验收依据见 2.5 节与 `04_演示项目定义.md` |
...
| v2.1 | 2026-08-17 | 补现状与残损记录、图纸样式设置、构件库条目复用三项功能；页面归属改为界面文档定义的十个视图 |
| v2.2 | 2026-08-20 | 2.5 补验收目标分层，展示阶段先行、商业指标试点阶段验证；4.1 版本规划同步；新增 2.8 定位收敛说明与立项书假设处置 |
| v2.3 | 2026-08-24 | 明确展示项目可携带正式构建环境产生的流程演示签发记录；浏览器本机仍无签发权限；高都采用按测绘流程设定的演示实测记录，并在界面持续说明数据边界 |
| v2.4 | 2026-08-25 | 统一 README、部署说明和产品文档中的展示边界；把展示签发记录、真实工程签发和本机权限分开表述 |

### 1.1 需求摘要

古建保护成果工作台面向承担测绘与保护成果生产的团队。产品把一栋建筑的照片、实测记录、历史图纸和任务要求整理为可追溯的建筑数据、三维模型、测绘图纸和成果档案。

AI 负责理解资料、分析问题和推进流程。确定性程序负责几何计算与格式生成，专业人员负责判断与责任签发。当前版本交付可复查、可更新的单栋建筑档案。
...
- **展示阶段（当前）**：验收对象是从任务书到成果归档的完整流程。每个环节均有可操作页面和由运行时生成的文件，可用于求职作品展示和商业演示。展示项目中的数值、责任角色和流程演示签发记录不属于真实工程事实。验收标准见 `04_演示项目定义.md` 第 2 节与第 6 节。
...
| 缩短单栋交付周期 | 从资料入库到待签发成果输出的工作日 | 较人工基线缩短 50% 以上 | 同上 |
...
产品定位从“面向区县文物部门及乙方生态的 AI 古建档案引擎”收敛为“面向测绘与文保勘察团队的成果生产工作台”。第一用户由区县文物部门改为成果生产方，产品形态由平台改为本地单机工作台。

旧定位的 MRD 与立项书于 2026-08-16 归档至 `../99_历史归档/01_文档历史版本/重分类归档_2026-08-16/`。本节只保留定位变化结果。
...
| 假设 | 内容 | 处置 | 跟踪落点 |
|---|---|---|---|
| H1 | 大模型能从古建照片识别部件并输出制图可用的结构化数据 | 部分验证后继续跟踪 | PoC 路线 1 已完成端到端验证，识别率量化仍需盲测（`../05_验证证据/02_PoC工作流验证/05_PoC路线1验证报告.md`）；识别能力与上限见 `../06_研究底稿/01_AI与制图能力边界.md` |
| H2 | 乙方愿为图纸生产工具付费 | 未验证，留待试点阶段 | 验证方式为一家乙方真实项目对照与付费意愿摸底；不影响展示阶段验收 |
...
| 版本 | 目标 | 范围 | 不做 |
|---|---|---|---|
| v1.0 | 从资料到待签发成果的完整生产流程，验收目标见 2.5 节 | 单机工作台，本地数据，成果归档 | 本机法定签发、多人在线协作 |
...
**本期做**：单栋建筑从资料到待签发成果的完整生产流程，含上表 18 项功能。公开展示项目可以随项目包载入正式构建环境生成的只读流程演示签发记录，用于呈现归档完成状态。

**本期不做**：

- 在浏览器本机完成法定签发与正式交付。本机环境没有可验证身份，只能核对项目包中的流程演示签发记录。
