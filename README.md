...
第一次访问会先显示两个项目的名称、规模和数据边界，再在后台载入约 42 MB 的完整项目包。高都项目完成后即可先进入，不需要等待清式样板载入完毕；具体时间取决于访问网络。

展示项目随项目包携带由正式构建环境产生的流程演示签发记录，用于展示归档完成后的页面与数据关系，不构成真实工程签发或交付资格。浏览器本地环境不能为新项目创建正式签发记录，修改已签发项目后也必须重新复核。
